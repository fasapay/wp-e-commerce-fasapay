...
if (isset($_POST['submit'])) {// The form is submitted
    ...
    if (get_smart('tj_ongkiropt') == 'yes' || $_POST['ongkirpkg'] != 'Gratis Ongkir')
    ...
    if ($validator->ValidateForm()) {
        ...
        if ($vid3[0]->id_order) {
            $id_order = $vid3[0]->id_order;
        } else {
            $id_order = substr(number_format(time() * rand(), 0, '', ''), 0, 9);
        }
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            if ($opsi) {
                $hargak = LapakInstan_Function::formatnom($opsi);
            } else if (LapakInstan_Function::proch($pdid)) {
                $hargak = LapakInstan_Function::proch($pdid);
            } else {
                $hargak = LapakInstan_Function::prich($pdid, $qty);
            }
        ...
        if (!$_POST['shippingaddress']) {
            ...
            if (get_smart('tj_showunik') == 'yes') {
                if ($pay_order == 'Transfer Bank') {
                    if (substr($_POST['totalorder'], -3) == 000) {
                        $angk = 3;
                    } else if (substr($_POST['totalorder'], -2) == 00) {
                        $angk = 2;
                    } else {
                        $angk = 1;
                    }
        ...
        $sortorder = '';
        if ($id_aff = $_COOKIE['smarttoko_ref_id']) {
            $aff_id = $id_aff;
        } else {
            $aff_id = 0;
        }
...
<?php if ($pser !== '01') { ?><div class="box-checkout box-shad">
    ...
    <div class="box-checkout box-shad late"><h2><span class="checkout-section-no"><i class="icon-shopping-cart"></i></span> Info Pesanan</h2>
        <div class="sumcart">
                    ...
                    <table cellpadding="5px" cellspacing="1px" class="ui-grid" id="checkoutCart" bgcolor="#f4f4f4">
    				 ...
    				 <tbody class="cartItems">';
                            ...
                            for ($i = 0; $i < $max; $i++) {
                                ...
                                if (strpos($opsi2, $harus) !== false) {
                                    $hargak = LapakInstan_Function::formatnom($opsi);
                                } else {
                                    if (LapakInstan_Function::proch($pid)) {
                                        $hargak = LapakInstan_Function::proch($pid);
                                    } else {
                                        $hargak = LapakInstan_Function::prich($pid, $q);
                                    }
                                }
                                $opsi3 = $_SESSION['cart'][$i]['opsinew'];
                                $opsi4 = $_SESSION['cart'][$i]['opsinew2'];
            ...
            <?php
            if ($opsi) {
                echo '<div class="opsipdcrt">' . $opsi2 . '</div>';
            }
            if ($opsi3) {
                echo '<div class="opsipdcrt">' . $opsi3 . '</div>';
            }
            if ($opsi4) {
                echo '<div class="opsipdcrt">' . $opsi4 . '</div>';
            }
            ?><div class="clear"></div>
                                            <div>
